...
-- Remove the policy that exposes raw mls_listings (including private_remarks) to anon
DROP POLICY IF EXISTS "Public can read active listings via view" ON public.mls_listings;

-- Recreate mls_listings_safe as SECURITY DEFINER so it doesn't need raw table access
