...
function interestedCta(opts: {
  ...
  projectName?: string;
  /** Email headline — used as fallback identifier when projectName is hidden/empty (so we never reveal a project we chose to hide) */
  headline?: string;
  agentFirstName?: string;
  /** Email subject line — included in the WhatsApp message so the agent knows which email the lead is replying to */
  emailSubject?: string;
  /** Starting price (e.g. "$369,900") — included so the agent sees what offer caught the lead's eye */
  startingPrice?: string;
  font: string;
  style?: "gold-fill" | "pill" | "outline";
}): string {
...
<table width="600" cellpadding="0" cellspacing="0" border="0" class="email-container" style="max-width:600px;width:600px;background:#ffffff;border:1px solid #e0dbd3;">
  ...
  ${interestedCta({ showInterestedCta: data.showInterestedCta, interestedWhatsapp: data.interestedWhatsapp, projectName: data.projectName, headline: data.headline, agentFirstName: agent.full_name?.split(" ")[0], emailSubject: data.subjectLine, startingPrice: data.startingPrice, font: BODY_FONT, style: "gold-fill" })}
...
<table cellpadding="0" cellspacing="0" border="0" align="center" width="100%" class="email-container"
  ...
  ${interestedCta({ showInterestedCta: data.showInterestedCta, interestedWhatsapp: data.interestedWhatsapp, projectName: data.projectName, headline: data.headline, agentFirstName: agent.full_name?.split(" ")[0], emailSubject: data.subjectLine, startingPrice: data.startingPrice, font: F, style: "outline" })}
