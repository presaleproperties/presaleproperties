...
import { EmailBlockPalette, BLOCK_CONFIGS, type BlockConfig } from "./EmailBlockPalette";
...
export function EmailTemplateBuilder({ initialTemplate, onSave }: EmailTemplateBuilderProps) {
  ...
  const [blocks, setBlocks] = useState<EmailBlockData[]>(() => {
    // Try to parse existing content or start with default blocks
    if (initialTemplate?.html_content) {
      // If there's existing HTML, we'll start fresh (parsing HTML to blocks is complex)
      return [
        {
