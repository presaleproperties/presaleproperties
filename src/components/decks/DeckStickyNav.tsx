...
import { Phone, Menu, X, BookOpen, HelpCircle } from "lucide-react";
...
export function DeckStickyNav({ visible, activeSection, phoneNumber }: DeckStickyNavProps) {
  ...
  return (
    <>
      ...
      >
        {/* ── Sub-nav (top bar) ── */}
        <div className="bg-muted/80 backdrop-blur-sm border-b border-border/40">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 h-8 flex items-center justify-end gap-4">
            {SUB_SECTIONS.map((s) => (
              <button
                key={s.id}
        ...
        {/* ── Main nav ── */}
        <div className="bg-background border-b border-border/70 shadow-[0_2px_12px_-2px_hsl(var(--foreground)/0.08)]">
              ...
              <div className="border-t border-border/40 pt-2 flex gap-2">
                {SUB_SECTIONS.map((s) => (
                  <button
                    ...
                    onClick={() => scrollTo(s.id)}
                    className={cn(
                      "inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium transition-all touch-manipulation",
                      activeSection === s.id
