    ...
    <div className="border-y border-border/50 bg-background">
      <div className="container px-6 sm:px-8">
        <div className="flex items-stretch overflow-x-auto scrollbar-hide divide-x divide-border/40">
          <div className="shrink-0 flex items-center pr-8 py-5">
            <span className="text-xs font-black uppercase tracking-[0.22em] text-muted-foreground whitespace-nowrap">Browse by City</span>
          </div>
          {CITIES.map((city) => {
            ...
            return (
              <Link
                key={city.name}
                to={`/${slug}`}
                className="shrink-0 flex items-center px-7 py-5 text-base font-semibold text-muted-foreground hover:text-foreground hover:bg-muted/40 transition-colors whitespace-nowrap"
              >
                {city.name}
              </Link>
            );
