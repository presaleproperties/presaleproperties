...
export function NeighborhoodProjectsCarousel({ 
  neighborhood, 
  city, 
  title, 
  subtitle, 
  excludeSlug 
}: NeighborhoodProjectsCarouselProps) {
  const scrollRef = useRef<HTMLDivElement>(null);
  const [canScrollLeft, setCanScrollLeft] = useState(false);
  const [canScrollRight, setCanScrollRight] = useState(true);

  const { data: projects, isLoading } = useQuery({
    queryKey: ["neighborhood-projects", neighborhood, city, excludeSlug],
    queryFn: async () => {
      let query = supabase
      ...
      return excludeSlug && data && data.length > 6 ? data.slice(0, 6) : data;
    },
  });
  ...
  // Don't render if less than 2 projects in neighborhood
  if (isLoading) {
    return (
  ...
  // Need at least 2 projects to show carousel
    ...
    <div className="space-y-4 md:space-y-5">
      ...
      >
        {projects.map((project) => (
          <div key={project.id} className="shrink-0 w-[calc(100vw-80px)] sm:w-[260px] md:w-[280px] lg:w-[300px] scroll-snap-start">
            <PresaleProjectCard
              id={project.id}
              slug={project.slug}
              name={project.name}
