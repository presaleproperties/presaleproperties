...
interface ExpertAdvisoryCardProps {
  onContactClick?: () => void;
}
...
export function ExpertAdvisoryCard({ onContactClick }: ExpertAdvisoryCardProps) {
  const handleContact = () => {
    if (onContactClick) {
      onContactClick();
    } else {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 sm:p-6 shadow-card">
      ...
      {/* CTA */}
      <Button onClick={handleContact} className="w-full h-12 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold text-base gap-2">
