...
import { ChevronRight, Building2, Home, DollarSign, TrendingDown } from "lucide-react";
...
export default function PresaleCityTypePricePage() {
  ...
  const metaTitle = `Presale ${productLabel} Under ${priceLabel} in ${cityName} (2026 Guide)`;
  const metaDescription = `Browse all presale ${productType}s under ${priceLabel} in ${cityName}, BC. Updated pricing, floor plans, deposit structures & expert guidance.`;
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 5, "name": `Under ${priceLabel}`, "item": `https://presaleproperties.com/presale-projects/${citySlug}/${typePriceSlug}` }
    ]
  };
  ...
  const faqs = [
    ...
    {
      question: `Are affordable ${cityName} ${productType}s a good investment?`,
      answer: `Entry-level presales can offer strong appreciation as buyers seek affordability. Consider transit access, completion timeline, and rental potential.`
    }
  ];
      ...
      <main className="flex-grow">
        ...
        <section className="container px-4 py-8 md:py-12">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-3xl md:text-4xl font-bold">Presale {productLabel} Under {priceLabel} in {cityName}</h1>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            ...
            {PRICE_POINTS.map((price) => (
              <Link 
                key={price} 
                to={`/presale-projects/${citySlug}/${typeSlug}-under-${price}k`}
              >
                <Button 
          ...
          <div className="flex gap-2 mb-8">
            <Link to={`/presale-projects/${citySlug}/condos-under-${priceK}k`}>
              <Button variant={isCondos ? "default" : "outline"} size="sm">
                <Building2 className="h-4 w-4 mr-2" />
                Condos Under {priceLabel}
              </Button>
            </Link>
            <Link to={`/presale-projects/${citySlug}/townhomes-under-${priceK}k`}>
              <Button variant={!isCondos ? "default" : "outline"} size="sm">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 bg-muted/30 rounded-xl">
              ...
              <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                We don't currently have {cityName} {productType}s with starting prices under {priceLabel}. 
              ...
              <div className="flex flex-wrap justify-center gap-2">
                ...
                <Link to={`/presale-projects/${citySlug}/${typeSlug}`}>
                  <Button>All {cityName} {productLabel}</Button>
                </Link>
              </div>
            </div>
          )}
        ...
        <section className="container px-4 py-12 border-t">
          <h2 className="text-xl font-semibold mb-6">
            {productLabel} Under {priceLabel} in Other Cities
          </h2>
          <div className="flex flex-wrap gap-2">
            {otherCities.map(([slug, name]) => (
              <Link key={slug} to={`/presale-projects/${slug}/${typeSlug}-under-${priceK}k`}>
                <Button variant="outline" size="sm">
                  {name}
                </Button>
              </Link>
        ...
        <section className="container px-4 py-12 border-t">
          <h2 className="text-2xl font-bold mb-6">
            FAQ: Affordable Presale {productLabel} in {cityName}
          </h2>
        ...
        <section className="container px-4 py-12 border-t">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Link to={`/presale-projects/${citySlug}/${typeSlug}`} className="text-sm text-primary hover:underline">
              {cityName} {productLabel}
            </Link>
