...
const UNIT_TYPES = ["Studio", "1 Bed", "2 Bed", "2 Bed + Den", "3 Bed", "Townhouse"];
...
export default function DashboardDeckBuilder() {
  ...
  const applyProject = useCallback((p: PresaleProject) => {
    ...
    // Hero image
    if (p.featured_image && !heroImageUrl) {
      setHeroImageUrl(p.featured_image);
    }

    // Gallery — merge project images in, deduplicate
    if (p.gallery_images?.length || p.featured_image) {
      const imgs = [...(p.featured_image ? [p.featured_image] : []), ...(p.gallery_images || [])];
      const unique = Array.from(new Set(imgs)).filter(Boolean).slice(0, 12);
      setGallery(unique);
    }

    // Tagline
    if (!tagline) {
      setTagline(`Presale Opportunity — ${p.city || "BC"}${p.completion_year ? ` · ${p.completion_year}` : ""}`);
    }

    toast.success(`Loaded "${p.name}" — all fields auto-populated. Customize as needed.`);
    ...
    <DashboardLayout>
      ...
      <div className="max-w-3xl mx-auto space-y-6 pb-16">
        ...
        <Section
          ...
          subtitle="Core details shown in the deck header and stats"
        ...
        <Section
          title="Hero Image"
          subtitle="Full-width background for the deck cover"
                ...
                {heroUploading ? (
                  ...
                  <>
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-muted-foreground">{linkedProjectId ? "Or select a project above to pull featured image automatically" : "Full-width background for the deck"}</p>
                    </div>
                  </>
                )}
              </div>
        ...
        {/* Section: Floor Plans */}
        <Section title={`Floor Plans (${floorPlans.length}/6)`} subtitle="Unit types, sizes, pricing and images">
          <div className="space-y-4">
        ...
        {/* Section: Gallery */}
        <Section
          title={`Photo Gallery (${gallery.length}/12)`}
        ...
        {/* Section: Projections */}
        <Section title="Investment Projections" subtitle="Rental estimates, cap rates and appreciation forecast">
        ...
        <Section title="Contact Information" subtitle="Agent contact details shown on the deck" defaultOpen={false}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
